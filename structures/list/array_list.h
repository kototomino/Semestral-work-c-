...
namespace structures
{
	...
	{
		// identita, typov· kompatibilita, porovnaù size, porovnaù prvky
		if (this == &other) {
			return true;
		}
		else {
	...
	template<typename T>
	inline T& ArrayList<T>::at(int index) //pristupovanie k prvku
	{
	...
	template<typename T>
	inline void ArrayList<T>::insert(const T& data, int index) //vkladanie na zaËiatok a do stredu
	{
	...
	inline void ArrayList<T>::enlarge()
	{
		...
		Array<T>* newBiggerArray = new Array<T>(size_*2);
		Array<T>::copy(*array_,0,*newBiggerArray, 0, size_); 
		delete array_;
		array_ = newBiggerArray; 
